...
(async () => {
  ...
  async function handleLanguageChange() {
    ...
    if (!languagesByName[newLanguageName]) {
      const url = `./tree-sitter-parser.wasm`
    ...
    for (let i = 0;; i++) {
      ...
      if (visitedChildren) {
        ...
        if (displayName) {
          ...
          */
          content = start.row === end.row ? cursor.nodeText : '';
          row = `<div>${'  '.repeat(indentLevel)}${fieldName}<a class='plain' href="#" data-id=${id} data-range="${start.row},${start.column},${end.row},${end.column}">${displayName}</a> <b>${content}</b>`;
          finishedRow = true;
        } else {
          row = `<div>${'  '.repeat(indentLevel)}TOK <b>${cursor.nodeText}</b>`;
          finishedRow = true;
        }
